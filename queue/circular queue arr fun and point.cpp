...
int isFull() {
    return ((rear + 1 - queue) % MAX_SIZE) == (front - queue);
}
...
void enqueue(int item) {
    if (isFull()) {
        ...
        *rear = item;
        rear = queue + ((rear - queue + 1) % MAX_SIZE);
...
int dequeue() {
    ...
    if (isEmpty()) {
        ...
        item = *front;
        front = queue + ((front - queue + 1) % MAX_SIZE);
...
void display() {
    if (isEmpty()) {
        ...
        while (ptr != rear) {
            printf("%d ", *ptr);
            ptr = queue + ((ptr - queue + 1) % MAX_SIZE);
        }
        printf("\n");
    }
}

// Function to deallocate memory allocated for the circular queue
void destroyQueue() {
    free(queue);
}
